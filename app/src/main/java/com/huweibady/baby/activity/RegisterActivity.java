...
public class RegisterActivity extends BaseActivity {
    ...
    /**
     * activity自身，为了让RegisterNextactivity注册成功后关闭这个
     */
    ...
    private void requestNext(final String sPhone, final String identifying) {
        ...
        if (TextUtils.isEmpty(sPhone) || TextUtils.isEmpty(identifying)) {
            Toast.makeText(RegisterActivity.this, "号码或验证码不能为空", Toast.LENGTH_SHORT).show();
            return;
        }
...
//                    intent.putExtra("identifying", identifying);
    ...
    private void requestIdentifying(String sPhone) {
        ...
        if (sPhone.length() != 11) {
            ...
            Toast.makeText(RegisterActivity.this, "手机号格式不正确", Toast.LENGTH_SHORT)
                    .show();
            return;
        }
        ...
        final Request request = new Request.Builder().url(Constants.BASE_URL2 + "messages?mob=" + sPhone).build();
...
//                id = Integer.parseInt(sId);
            ...
            } else if (sta == 600) {//已经注册过了

                RegisterActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {

                        Toast.makeText(RegisterActivity.this, "号码已注册", Toast.LENGTH_SHORT).show();
                    }
                });
                ...
                RegisterActivity.this.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Toast.makeText(RegisterActivity.this, "服务器出错，请重试", Toast.LENGTH_SHORT).show();
                    }
                });
            }
